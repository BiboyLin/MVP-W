...
│
├── components/
│   ├── sensecap-watcher        # 硬件抽象 (官方SDK)
│   │   ├── bsp_audio_init()  # 音频初始化
│   │   ├── bsp_lvgl_init()   # LVGL 初始化
│   │   └── bsp_knob_btn_init() # 按钮初始化
│   │
│   ├── sscma_client          # Himax 通信
│   └── opus                   # Opus 编解码
│
└── CMakeLists.txt
```
...
> 使用 Watcher-Firmware 官方 SDK 中的 LVGL
...
│  ┌─────────────────────────────────────┐                   │
│  │  LVGL 显示                           │                   │
│  │  - 显示文字 (lv_label_set_text)      │                   │
│  │  - 显示表情图标 (lv_img)             │                   │
│  └─────────────────────────────────────┘                   │
│                                                              │
│  支持的表情:                                                  │
│  ┌────────┬────────┬────────┬────────┬────────┐          │
│  │  happy │  sad  │ surprised│ angry │  normal │          │
│  │   😊   │   😢   │    😲    │   😠   │   😐    │          │
│  └────────┴────────┴────────┴────────┴────────┘          │
│                                                              │
└─────────────────────────────────────────────────────────────┘
...
│   ├── audio_handler.py      # 音频流处理
│   ├── video_handler.py      # 视频流处理
│   └── control_handler.py   # 控制指令
│
├── ai/
│   ├── asr.py               # 语音识别 (可选)
│   ├── llm.py               # LLM 接口
